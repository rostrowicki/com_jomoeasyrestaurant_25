...
$menuselect = JFormHelper::loadFieldType('MenuSelect', false);
$menuOptions = $menuselect->getOptions(); // works only if you set your field getOptions on public!!
?>

<form action="<?php echo JRoute::_('index.php?option=com_jomoeasyrestaurantmenu&amp;view=categories'); ?>" method="post" name="adminForm" id="adminForm">
    <fieldset id="filter-bar">
        ...
        <div class="filter-select fltrt">
            <select name="filter_menu_id" class="inputbox" onchange="this.form.submit()">
                <option value=""><?php echo JText::_('JOPTION_SELECT_MENU'); ?></option>
                <?php echo JHtml::_('select.options', $menuOptions, 'value', 'text', $this->state->get('filter.menu_id')); ?>
            </select>            
    ...
    <table class="adminlist">
        <thead>
            <tr>
                ...
                <th><?php echo JText::_('COM_JOMOEASYRESTAURANTMENU_COLUMNS') ?></th>
                <th width="5%">
...
//                $item->cat_link = JRoute::_('index.php?option=com_categories&extension=com_weblinks&task=edit&type=other&cid[]=' . $item->menu_id);
                $canCreate = $user->authorise('core.create', 'com_jomoeasyrestaurantmenu.category.' . $item->menu_id);
                $canEdit = $user->authorise('core.edit', 'com_jomoeasyrestaurantmenu.category.' . $item->menu_id);
                $canCheckin = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $user->get('id') || $item->checked_out == 0;
                $canChange = $user->authorise('core.edit.state', 'com_jomoeasyrestaurantmenu.category.' . $item->menu_id) && $canCheckin;
                ?>
                <tr class="row<?php echo $i % 2; ?>">
                    ...
                    <td class="order">
                        <?php if ($canChange) : ?>
                            <?php if ($saveOrder) : ?>
                                <?php if ($listDirn == 'asc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($item->menu_id == @$this->items[$i - 1]->menu_id), 'categories.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, ($item->menu_id == @$this->items[$i + 1]->menu_id), 'categories.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php elseif ($listDirn == 'desc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($item->menu_id == @$this->items[$i - 1]->menu_id), 'categories.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, ($item->menu_id == @$this->items[$i + 1]->menu_id), 'categories.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php endif; ?>
                            <?php endif; ?>
                    ...
                    <td width="3%" class="right"><?php echo $this->escape($item->layout) ?></td>
                    <td class="center">
